...
trait BasePage extends PageObject with Eventually with Matchers with LazyLogging with BrowserDriver {
  ...
  object Locators {
    ...
    val linkServiceNavigation          = By.ByClassName("govuk-service-navigation__link")
  ...
  def clickConfirmAndSubmit(): Unit = click(By.xpath(Locators.btnContinueConfirm))
...
//  def removeFile(): Unit        = click(By.cssSelector(Locators.lnkRemoveFile))
  ...
  def clickServiceNavLink(): Unit        = click(Locators.linkServiceNavigation)
